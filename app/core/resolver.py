"""
Home Memory - Path Resolution Logic

This module provides pure functions for resolving paths and names
to Container and Item instances. No business logic here—just lookups.

Resolution is deterministic and never involves AI/LLM.
All ambiguity is surfaced as structured errors for the caller to handle.
"""
...
from app.db.models import Container, Item, to_canonical_name
...
def get_effective_security_level(
    session: Session,
    container: Container,
) -> int:
    """
    Get the effective security level for a container.
    
    Security level is inherited - the effective level is the
    maximum of the container's own level and all its ancestors.
    ...
    Returns:
        Effective security level (0, 1, or 2)
    """
    ancestors = get_container_ancestors(session, container)
    all_levels = [container.security_level] + [a.security_level for a in ancestors]
    return max(all_levels)


def is_container_accessible(
    session: Session,
    container: Container,
    max_security_level: int = 0,
) -> bool:
    """
    Check if a container is accessible at a given security level.
    
    Args:
        session: SQLAlchemy session
        container: Container to check
        max_security_level: Maximum allowed security level (0 = open only)
        
    Returns:
        True if container can be accessed at given security level
    """
    effective_level = get_effective_security_level(session, container)
    return effective_level <= max_security_level
